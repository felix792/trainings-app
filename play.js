...
function persistDrawing() {
  ...
  const pl  = t && (t.plays || []).find((p) => p.id === playId);
  if (!pl) return;
  pl.drawing = elements;
  saveTeams(all);
}
...
const play = team && (team.plays || []).find((p) => p.id === playId);
...
if (!play) {
  ...
  document.getElementById('playMeta').textContent     = team.name;

  elements  = play.drawing   ? JSON.parse(JSON.stringify(play.drawing)) : [];
...
function initBoard() {
  ...
  document.getElementById('downloadBoard').addEventListener('click', () => {
    ...
    link.download = (play.name || 'play').replace(/[^a-z0-9]/gi, '_') + '.png';
...
// ── Notes ──
function initNotes() {
  const textarea = document.getElementById('playNotes');
  const status   = document.getElementById('notesStatus');
  textarea.value = play.notes || '';
  let debounce;
